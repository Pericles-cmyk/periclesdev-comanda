...
ensureLocalInfrastructure().then(()=>{setInterval(backupSchedulerTick,60_000);backupSchedulerTick();server.listen(PORT,'0.0.0.0',()=>console.log(`ComandaWeb Local: http://0.0.0.0:${PORT}`));}).catch(e=>{console.error('Falha ao iniciar infraestrutura local:',e);process.exit(1)});
